...
extension NutritionLabelView {
    ...
    func nutrientRow(forType type: NutrientType, indentLevel: Int = 0, prefixedWithIncludes: Bool = false) -> some View {
        ...
        if let value = dp.nutrient(ofType: type) {
            row(title: title,
                prefix: prefix,
                value: value,
...
//        if let value = dp.nutrient(ofCustomType: type) {
//            row(title: type.name ?? "",
//                value: value,
...
//    }
    ...
    var fatRows: some View {
        Group {
            row(title: "Total Fat", value: dp.fatAmount, rdaValue: MacroRDA.fat, unit: "g", bold: true)
    ...
    var carbRows: some View {
        Group {
            row(title: "Total Carbohydrate", value: dp.carbAmount, rdaValue: MacroRDA.carb, unit: "g", bold: true)
